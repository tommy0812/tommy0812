...
class CreateAnno:
    ...
    def add_filename(self, filename_text_str='00000.jpg'):
        print("filename_text_str:",filename_text_str)
        if filename_text_str.endswith(".jpg"):
            prefix = filename_text_str.split(".jpg")[0]
            filename_text_str = prefix + ".png"
            print("Replace jpg to png:",filename_text_str)
    ...
    def add_object(self,
        ...
        name = self.doc.createElement('category')
        name_text = self.doc.createTextNode(name_text_str)
        name.appendChild(name_text)
        object.appendChild(name)

        # print("name_text_str:",name_text_str)
        # print("SEG_str:",seg_text_str)
        ...
        type_ = self.doc.createElement('type')
        type_text = self.doc.createTextNode(name_text_str)
        type_.appendChild(type_text)
        object.appendChild(type_)

        attribution = self.doc.createElement('attribution')
        attribution_text = self.doc.createTextNode(name_text_str)
        attribution.appendChild(attribution_text)
        object.appendChild(attribution)

        segmentation = self.doc.createElement('segmentation')
        segmentation_text = self.doc.createTextNode(seg_text_str)
        segmentation.appendChild(segmentation_text)
        object.appendChild(segmentation)
